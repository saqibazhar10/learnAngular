...
handleFileUpload(event: Event): void {
  ...
  if (file) {
    ...
    this.http.post('http://ec2-16-171-66-13.eu-north-1.compute.amazonaws.com:8000/upload', formData).subscribe({
      next: (response) => {
...
triggerDocChat() {
  ...
  fetchData(): void {
    this.http.get<any>('http://ec2-16-171-66-13.eu-north-1.compute.amazonaws.com:8000/documents', {
  headers: {
...
deletefile(doc_id:number):void{
  this.http.delete<any>(`http://ec2-16-171-66-13.eu-north-1.compute.amazonaws.com:8000/documents/${doc_id}`).subscribe(
      (response) => {
        console.log('API response:', response);
        this.fetchData();
      },
